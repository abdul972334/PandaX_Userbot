...
# --------------------------------------------------------------------#
telegraph = Telegraph()
r = telegraph.create_account(short_name="PetercordPanda")
auth_url = r["auth_url"]
# --------------------------------------------------------------------#
...
async def update(eve):
    ...
    if Var.HEROKU_API:
        ...
        if not heroku_app:
            ...
            return
        await eve.edit(
            "`proses pembaruan panda userbot segera dimulai, mohon tunggu beberapa saat.`"
        )
        ups_rem.fetch(ac_br)
        ...
        try:
            ...
            return
        await eve.edit(
            "`petercordpanda userbot telah diperbarui!\nmemulai ulang, mohon tunggu...`"
        )
    else:
        await eve.edit(
            "`proses pembaruan panda userbot segera dimulai, mohon tunggu beberapa saat.`"
        ...
        await eve.edit(
            "`panda userbot telah diperbarui!\nmemulai ulang, mohon tunggu...`"
        )
        execl(sys.executable, sys.executable, "-m", "pyPetercordPanda")
...
async def _(e):
    ...
    await e.reply(
        "`Success!\nYou are all set to use Google Drive with Petercordpanda Userbot.`",
...
async def _(e):
    ...
    async with petercordpanda_bot.asst.conversation(e.sender_id) as conv:
...
async def otvaar(event):
    await event.edit(
        "other variables to set for @TEAMSquadUserbotSupport:",
        buttons=[
            [
...
async def emoji(event):
    ...
    async with event.client.conversation(pru) as conv:
        await conv.send_message(
            "🐼 kirim emoji untuk di set di menu help.\n\ntekan /cancel untuk membatalkan."
        )
