...
if addons == "True" or addons is None:
    try:
        os.system(
            "git clone https://github.com/IlhamMansiez/PetercordPandaAddons.git addons/"
        )
    except BaseException:
        pass
    ...
    for plugin_name in files:
        try:
            if plugin_name.endswith(".py"):
                ...
                if not plugin_name.startswith("__") or plugin_name.startswith("_"):
                    LOGS.info(f"🐼 PandaUserbot - Addons -  Installed - {plugin_name}")
        except Exception:
            LOGS.info(f"🐼 PandaUserbot - Addons - ERROR - {plugin_name}")
            LOGS.info(str(traceback.print_exc()))
else:
