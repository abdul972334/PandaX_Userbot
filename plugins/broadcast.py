...
async def broadcast_adder(event):
    ...
    if msgg == "all":
        ...
        chats = [
            e.entity
            for e in await petercordpanda.get_dialogs()
...
async def list_all(event):
    ...
    for channel in channels:
        name = ""
        try:
            name = (await petercordpanda.get_entity(int(channel))).title
        except BaseException:
            name = ""
        msg += f"=> **{name}** [`{channel}`]\n"
...
async def forw(event):
    ...
    for channel in channels:
        try:
            await petercordpanda_bot.forward_messages(int(channel), previous_message)
            sent_count += 1
            await x.edit(
                f"Sent : {sent_count}\nError : {error_count}\nTotal : {len(channels)}",
            )
