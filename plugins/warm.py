...
@ilhammansiz_cmd(pattern="warn ?(.*)", groups_only=True, admins_only=True)
async def warn(e):
    ...
    if count + 1 >= number:
        if "ban" in action:
            ...
            try:
                await petercordpanda_bot.kick_participant(e.chat_id, user)
            except BaseException:
                return await eod(e, "`Something Went Wrong.`")
        elif "mute" in action:
            try:
...
@ilhammansiz_cmd(pattern="resetwarn ?(.*)", groups_only=True, admins_only=True)
async def rwarn(e):
    reply = await e.get_reply_message()
    if reply:
        user = reply.from_id.user_id
...
@ilhammansiz_cmd(pattern="warns ?(.*)", groups_only=True, admins_only=True)
async def twarns(e):
    reply = await e.get_reply_message()
    if reply:
        user = reply.from_id.user_id
...
@ilhammansiz_cmd(pattern="setwarn ?(.*)")
async def warnset(e):
