...
async def kck(ult):
    ...
    if user.id == petercordpanda_bot.uid:
        return await xx.edit("`kamu tidak bisa kick dirimu sendiri.`")
    try:
        await petercordpanda_bot.kick_participant(ult.chat_id, user.id)
...
async def pin(msg):
    ...
    if not msg.is_private:
        link = (await petercordpanda_bot(ExpLink(msg.chat_id, xx))).link
        mss = f"`menyematkan` [pesan ini]({link})"
    ch = msg.pattern_match.group(1)
    if ch != "silent":
        slnt = True
        try:
            await petercordpanda_bot.pin_message(msg.chat_id, xx, notify=slnt)
        except BadRequestError:
        ...
        try:
            await petercordpanda_bot.pin_message(msg.chat_id, xx, notify=False)
        except BadRequestError:
            return await eor(msg, "`hmmm, saya pikir saya tidak punya izin disini!`")
        except Exception as e:
            return await eor(msg, f"**ERROR:**`{str(e)}`")
...
async def unp(ult):
    ...
    if msg and not ch:
        try:
            await petercordpanda_bot.unpin_message(ult.chat_id, msg)
        except BadRequestError:
        ...
        try:
            await petercordpanda_bot.unpin_message(ult.chat_id)
        except BadRequestError:
            return await xx.edit("`hmmm, saya pikir saya tidak punya izin disini!`")
        except Exception as e:
...
async def fastpurger(purg):
    ...
    async for msg in petercordpanda_bot.iter_messages(chat, min_id=purg.reply_to_msg_id):
        ...
        if len(msgs) == 100:
            await petercordpanda_bot.delete_messages(chat, msgs)
            msgs = []

    if msgs:
        await petercordpanda_bot.delete_messages(chat, msgs)
    done = await petercordpanda_bot.send_message(
        purg.chat_id,
...
async def fastpurgerme(purg):
    ...
    if num and not purg.is_reply:
        try:
            ...
            return
        mp = 0
        async for mm in petercordpanda_bot.iter_messages(
...
async def _(e):
    ...
    if e.reply_to_msg_id:
        ...
        try:
            await petercordpanda_bot(DeleteUserHistoryRequest(e.chat_id, input))
...
@ilhammansiz_cmd(pattern="pinned")
...
@ilhammansiz_cmd(pattern="listpinned")
...
@ilhammansiz_cmd(pattern="autodelete ?(.*)", groups_only=True, admins_only=True)
async def autodelte(ult):  # Tg Feature
    ...
    if match == "24h":
        ...
        tt = 0
    try:
        await petercordpanda_bot(SetHistoryTTLRequest(ult.chat_id, period=tt))
