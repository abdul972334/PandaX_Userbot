...
from PandaX_Userbot.misc import sudoers
...
async def _(ult):
    ...
    if ult.reply_to_msg_id:
        ...
        try:
            ...
            name = ""
        if id == petercordpanda_bot.me.id:
            mmm += "kamu tidak dapat menambahkan dirimu sendiri sebagai anggota sudo...Goblok 😑"
        elif is_sudo(id):
            if name != "":
...
async def _(ult):
    ...
    if str(ult.sender_id) in sudoers():
        return await eod(
            ult,
            "kamu anggota sudo, tidak dapat menghapus anggota sudo lain. Goblok 😑",
        )
    ok = await eor(ult, "`memperbarui daftar anggota sudo...`")
    mmm = ""
    if ult.reply_to_msg_id:
