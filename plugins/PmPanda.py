...
if Redis("PMPIC"):
    ...
    PMPIC = "resources/extras/teampetercordpanda.jpg"
    ...
    async def autoappr(e):
        ...
        if not is_approved(e.chat_id):
            ...
            async for message in e.client.iter_messages(e.chat_id, search=UNS):
                await message.delete()
            try:
                await petercordpanda_bot.edit_folder(e.chat_id, folder=0)
            except BaseException:
                pass
    ...
    async def permitpm(event):
        ...
        if not apprv and event.text != UND:
            if Redis("MOVE_ARCHIVE") == "True":
                try:
                    await petercordpanda.edit_folder(user.id, folder=1)
                except BaseException:
                    pass
            if event.media:
                await event.delete()
            ...
            if user.id in LASTMSG:
                ...
                if event.text != prevmsg:
                    if "pesan masuk" in event.text:
                        return
                    async for message in petercordpanda.iter_messages(
                        user.id,
                        search=UND,
                    ):
                        await message.delete()
                    ...
                    await petercordpanda.send_file(
                        user.id,
                        PMPIC,
                        caption=message_,
                    )
                ...
                async for message in petercordpanda.iter_messages(user.id, search=UND):
                    await message.delete()
                message_ = UNAPPROVED_MSG.format(
                    ON=OWNER_NAME,
                    warn=wrn,
            ...
            if COUNT_PM[user.id] >= WARNS:
                ...
                await petercordpanda(BlockRequest(user.id))
                await petercordpanda(ReportSpamRequest(peer=user.id))
                name = await petercordpanda.get_entity(user.id)
                name0 = str(name.first_name)
                await asst.edit_message(
                    int(udB.get("LOG_CHANNEL")),
                    _not_approved[user.id],
...
async def apr_in(event):
    ...
    if not is_approved(uid):
        approve_user(uid)
        try:
            await petercordpanda_bot.edit_folder(uid, folder=0)
        except BaseException:
            pass
        try:
            user_name = (await petercordpanda.get_entity(uid)).first_name
        except BaseException:
            user_name = ""
        await event.edit(
        ...
        async for message in petercordpanda.iter_messages(uid, search=UND):
            await message.delete()
...
async def unblck_in(event):
    uid = int(event.data_match.group(1).decode("UTF-8"))
    await petercordpanda(UnblockRequest(uid))
    try:
        user_name = (await petercordpanda.get_entity(uid)).first_name
    except BaseException:
        user_name = ""
