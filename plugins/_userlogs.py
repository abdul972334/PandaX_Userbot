...
async def all_messages_catcher(e):
    if udB.get("TAG_LOG"):
        ...
        send = await petercordpanda_bot.get_messages(e.chat_id, ids=e.id)
        try:
            if x.username:
...
async def _(e):
    ...
    y = await petercordpanda_bot.get_entity(int(wah))
...
@petercordpanda.on(events.ChatAction)
...
async def leave_ch_at(event):
    ...
    if client == "bot":
        ...
        name = (await petercordpanda_bot.get_entity(int(ch_id))).title
        await petercordpanda_bot.delete_dialog(int(ch_id))
